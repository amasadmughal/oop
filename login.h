...
void LoginSignup::SignUp()
{
	...
	cin.ignore();
	string fav = "NULL";
	cout << "Enter your name without space & in lower case\n";
	...
	for (int i = 0; i < names.length(); i++)
	{
		if (names[i] == ' ')
		{
			cout << "write a name without space\n";
			cout << "Username: ";
			getline(cin, names, '\n');
		}
	}
	...
	{
		//system("cls");
		cout << "Username Confirmed....\n\n";
	}



	//taking password*//
	cout << "Password [example asad1234 OR 12345678]:\n";
	getline(cin, pass, '\n');
...
void LoginSignup::PasswordValidation(string& pass)
{
	...
	for (int i = 0; i < pass.length(); i++)
	{
		if (pass[i] == ' ')
		{
			cout << "\nPassword must be without spaces [example 123445678 or asad6214]\n";
			cout << "Password: ";
			getline(cin, pass, '\n');
		}
	}
